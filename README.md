...
*   **Alpha Channel:** Optional transparency slider for RGBA support.
...
*   `showAlpha` (Boolean, default: `true`): Whether to display the alpha (transparency) slider.
...
*   `format` (String, default: `'auto'`): The format used to display the color in the text input. Possible values:
    *   `'auto'`: Shows `hex` if `alpha` is 1, otherwise shows `rgba(r, g, b, a)`.
    ...
    *   `'rgba'`: Always shows the color in RGBA format (e.g., `rgba(255, 0, 0, 1)` or `rgba(255, 0, 0, 0.5)`).
    *   `'hsl'`: Always shows the color in HSL format (e.g., `hsl(0, 100%, 50%)`). Alpha information is not displayed.
    *   `'hsla'`: Always shows the color in HSLA format (e.g., `hsla(0, 100%, 50%, 1)` or `hsla(0, 100%, 50%, 0.5)`).
...
*   **Popup Palette:**
    ...
    *   **Alpha Slider (Optional):** Drag horizontally to adjust transparency.
...
*   **Альфа-канал:** Опциональный ползунок прозрачности для поддержки RGBA.
...
*   `showAlpha` (Boolean, по умолчанию: `true`): Показывать ли ползунок альфа-канала (прозрачности).
...
*   `format` (String, по умолчанию: `'auto'`): Формат, используемый для отображения цвета в текстовом поле. Возможные значения:
    *   `'auto'`: Показывает `hex`, если `alpha` равен 1, иначе показывает `rgba(r, g, b, a)`.
    ...
    *   `'rgba'`: Всегда показывает цвет в формате RGBA (например, `rgba(255, 0, 0, 1)` или `rgba(255, 0, 0, 0.5)`).
    *   `'hsl'`: Всегда показывает цвет в формате HSL (например, `hsl(0, 100%, 50%)`). Информация об альфа-канале не отображается.
    *   `'hsla'`: Всегда показывает цвет в формате HSLA (например, `hsla(0, 100%, 50%, 1)` или `hsla(0, 100%, 50%, 0.5)`).
...
*   **Всплывающая палитра:**
    ...
    *   **Ползунок альфа-канала (опционально):** Перетаскивайте по горизонтали, чтобы регулировать прозрачность.
